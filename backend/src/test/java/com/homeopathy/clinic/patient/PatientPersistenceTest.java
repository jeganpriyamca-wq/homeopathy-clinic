...
class PatientPersistenceTest {
    ...
    @Test void searchByExactBirthDateSupportsBothFormatsAndRejectsInvalidDates() {
        create("Arun", "9876543210");
        create("Meena", "9876543211");
        service.create(new CreatePatientRequest(new PatientDetails("Other", "", java.time.LocalDate.of(1990, 5, 16), "9876543212", "", "Test Address"), false));
        service.create(new CreatePatientRequest(new PatientDetails("Unknown", "", null, "9876543213", "", "Test Address"), false));
        assertEquals(2, service.search("1990-05-15", 0, 20).totalElements());
        assertEquals(2, service.search(" 15/05/1990 ", 0, 20).totalElements());
        assertEquals(2, service.search("1990-05-15", 0, 1).totalPages());
        assertEquals(1, service.search("1990-05-15", 1, 1).items().size());
        assertEquals(0, service.search("31/02/1990", 0, 20).totalElements());
        assertEquals(0, service.search("1990-02-30", 0, 20).totalElements());
        assertEquals(0, service.search("15/05", 0, 20).totalElements());
    }
